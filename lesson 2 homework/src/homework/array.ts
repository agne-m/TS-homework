...
console.group('Užduotys');
{
    ...
    console.groupCollapsed('2. Sukurkite ir tipais aprašykite funkciją, kuri sudarytų string\'ą iš string\'ų masyvo elementų pirmųjų raidžių');
    {
        ...
        const strings: string[] = ['laughing', 'quietly ', 'to ', 'myself'];

        function returnStringFromFirstLetters(strings: string[]) {
            strings.forEach(string => {
                let result: string = string.charAt(0)
                console.log(result);
                return result //?
                /*
                  Pvz.:
